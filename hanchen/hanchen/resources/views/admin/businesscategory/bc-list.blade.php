...
@section('content')
	...
	<div class="text-c">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" onclick="admin_role_edit('添加分类','{{ url('admin/bc/create') }}','0','800','400')"><i class="Hui-iconfont">&#xe600;</i> 添加分类</a></span> <span class="r">共有数据：<strong>{{ $num }}</strong> 条</span> </div>
		...
		@for($i = 0;$i < $num;$i++ )
			<tr class="text-c">
				...
				<td>{{ $bcs[$i]->bc_name }}</td>
				<td>{{ $bcs[$i]->bc_time }}</td>
				<td class="f-14"><a title="编辑" onclick="admin_role_edit('分类名编辑','{{ url('admin/bc') }}/'+'{{ $bcs[$i]->bc_id }}','0','800','400')" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a> <a title="删除" href="javascript:;" onclick="admin_role_del(this,'{{ $bcs[$i]->bc_id }}')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
			@endfor
		</tbody>
	</table>
